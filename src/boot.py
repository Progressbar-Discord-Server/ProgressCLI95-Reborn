...
def main() -> None:
    ...
    # temporary solution
    while True:
        lvl = game.level.GameLevel(number=int(save_.get_value('LVL')), system='95')
        if lvl.play():
            continue
        else:
            save_.set_value('LVL', int(save_.get_value('LVL')) + 1)
            rprint(f'[italic]Press Enter to continue or type "quit" to quit: [/italic]', end='')
            choice = input()
